...
inline void GridSlamProcessor::scanMatch_mg(const double (*plainReading)[3]){
  ...
  for (ParticleVector::iterator it=m_particles.begin(); it!=m_particles.end(); it++){
    ...
    double score, l, s;
  //파티클마다 가중치가 가장 높게 잡힐 수 있는 위치로 다시 파티클의 위치를 정렬
    score=m_matcher.optimize_mg(corrected, it->map_mg, it->pose, plainReading);
    //    it->pose=corrected;
    ...
    //각 파티클의 위치에서 가지는 확률을 구하고 이를 각 파티클에 더해줌
    m_matcher.likelihoodAndScore_mg(s, l, it->map_mg, it->pose, plainReading);
    sumScore+=score;
    //위에서 구한 likelihood를 모두 더해준다.
    it->weight+=l;
    it->weightSum+=l;
...
inline bool GridSlamProcessor::resample_mg(const double (*plainReading)[3], int adaptSize, const MgReading* reading){
