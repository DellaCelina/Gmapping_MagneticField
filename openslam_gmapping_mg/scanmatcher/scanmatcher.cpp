...
double ScanMatcher::registerScan_mg(MgMatcherMap& map, const OrientedPoint& p, const double (*readings)[3]){
	return 0.0;
}
...
double ScanMatcher::optimize_mg(OrientedPoint& pnew, const MgMatcherMap& map, const OrientedPoint& init, const double (*readings)[3]) const{
	//파티클의 위치에서 각 앞, 뒤, 좌, 위 등등으로 델타를 추가하여 이중 가장 높은 가중치를 가지는 위치로 파티클의 위치를 옮긴다.
	double bestScore=-1;
	OrientedPoint currentPose=init;
	double currentScore=score_mg(map, currentPose, readings);
...
//		cout <<  "pose=" << currentPose.x  << " " << currentPose.y << " " << currentPose.theta << endl;
		...
		do {
			...
			double localScore=odo_gain*score_mg(map, localPose, readings);
